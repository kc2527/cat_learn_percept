# -*- coding: utf-8 -*-
"""
Readable CP + interleaved RSA protocol (lab mode, PsychoPy).

This version is intentionally laid out top-to-bottom so the experiment flow is easy to follow.
...
if __name__ == '__main__':
    ...
    base_meta = {
        ...
        'design': DESIGN,
        'seed': seed,
        'eeg_enabled': 1 if eeg.enabled else 0,
        'port_address': EEG_PORT_ADDRESS,
    ...
    write_row(writer, fhandle, base_meta, {'ts_iso': now_iso(), 'event': 'meta'})
    ...
    def show_message_wait_space(text: str, trigger_onset=None, event_onset='screen_onset', event_confirm='screen_confirm', payload=None):
        ...
        if trigger_onset is not None:
            ...
            win.flip()
            write_row(writer, fhandle, base_meta, {'ts_iso': now_iso(), 'event': event_onset, **({} if payload is None else payload)})
        ...
        while True:
            ...
            if 'escape' in keys:
                return False
            if 'space' in keys:
                write_row(writer, fhandle, base_meta, {'ts_iso': now_iso(), 'event': event_confirm, **({} if payload is None else payload)})
                return True
            core.wait(0.01)
        ...
        for i_block, item_id in enumerate(block_trials, start=1):
            ...
            write_row(
                ...
                base_meta,
                {
                    ...
                    'event': 'rsa_trial',
                    'trial_type': 'rsa',
                    'trial_index': runtime_state['rsa_trial_global'],
                    'rsa_point_index': point_idx,
                    'rsa_chunk_index': chunk_idx,
                    'rsa_block_index': block_idx,
    ...
    def run_rsa_point(point_idx: int, runtime_state):
        ...
        ok = show_message_wait_space(
            ...
            'This part can feel repetitive or boring, but please stay awake and keep looking at the center.\n\n'
        ...
        ok = show_message_wait_space(
            ...
            'You may wriggle and get comfortable again now.\n\n'
    ...
    # --------------------------- CP instructions --------------------------------
    ok = show_message_wait_space(
        'In each trial, you will see two intervals (stimulus pair 1 and stimulus pair 2).\n'
        'In one of these intervals, the two gratings are different. In the other, they are the same.\n'
        'The gratings can differ in bar thickness (spatial frequency), angle (orientation), or both.\n'
        ...
        'The main blocks are harder by design, and that is expected.\n'
    ...
    def run_one_cp_trial(trial_def, cp_main_index, cp_practice_index, block_id, post_rsa_idx):
        ...
        ok_local = run_phase(iti, 'fix', trig_code=TRIG['CP_ITI_ONSET'], trig_event='cp_iti_onset')
        if not ok_local:
            return False
        ...
        grating.ori = ori
        grating.phase = 0.0
        ok_local = run_phase(INTERVAL_SEC, 'stim', trig_code=TRIG['CP_INTERVAL1_ONSET'], trig_event='cp_interval1_onset')
        if not ok_local:
            return False

        # Main trials get condition-coded trigger (60s) at interval-1 onset.
        if trial_def['trial_type'] == 'main':
            family = runtime['cp_family']
            level = runtime['cp_distance_level']
            if family == 'within_A' and level == 'near':
                cond_code = TRIG['CP_STIM_WITHIN_A_NEAR']
            elif family == 'within_A' and level == 'far':
                cond_code = TRIG['CP_STIM_WITHIN_A_FAR']
            elif family == 'within_B' and level == 'near':
                cond_code = TRIG['CP_STIM_WITHIN_B_NEAR']
            elif family == 'within_B' and level == 'far':
                cond_code = TRIG['CP_STIM_WITHIN_B_FAR']
            elif family == 'between_AB' and level == 'near':
                cond_code = TRIG['CP_STIM_ACROSS_NEAR']
            else:
                cond_code = TRIG['CP_STIM_ACROSS_FAR']
        ...
        ok_local = run_phase(PAIR_GAP_SEC, 'blank')
        if not ok_local:
            return False
        ...
        grating.ori = ori
        grating.phase = 0.0
        ok_local = run_phase(INTERVAL_SEC, 'stim')
        if not ok_local:
            return False

        # ISI
        ok_local = run_phase(ISI_SEC, 'fix')
        if not ok_local:
            return False
        ...
        grating.ori = ori
        grating.phase = 0.0
        ok_local = run_phase(INTERVAL_SEC, 'stim', trig_code=TRIG['CP_INTERVAL2_ONSET'], trig_event='cp_interval2_onset')
        if not ok_local:
            return False
        ...
        # Log trial row
        interval1_changed = 1 if (abs(runtime['int1a']['x'] - runtime['int1b']['x']) + abs(runtime['int1a']['y'] - runtime['int1b']['y'])) > 1e-9 else 0
        interval2_changed = 1 if (abs(runtime['int2a']['x'] - runtime['int2b']['x']) + abs(runtime['int2a']['y'] - runtime['int2b']['y'])) > 1e-9 else 0
        write_row(
            writer,
            fhandle,
            base_meta,
            {
                ...
                'interval1_changed': interval1_changed,
                'interval2_changed': interval2_changed,
    ...
    ok = show_message_wait_space(
        ...
        'The main trials are harder, and that is expected.\n'
    ...
    # --------------------------- End -------------------------------------------
    draw_message('Thank you for being awesome and participating!!\nPress SPACE to exit.')
    win.flip()
    while True:
        eeg.update(default_clock)
        keys = event.getKeys(keyList=['space', 'escape'])
